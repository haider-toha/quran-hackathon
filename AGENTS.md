...
Project conventions live in `.Codex/skills/` — `backend-coding-standards`, `frontend-coding-standards`, `services-architecture`, `quran-domain`, `rag-patterns`, `supabase-migrations`. Read those before writing code in the matching area.
...
- When using Anthropic, the current default models are `Codex-sonnet-4-6` for generation and `Codex-haiku-4-5-20251001` for classification/routing. Treat these as defaults, not mandates.
