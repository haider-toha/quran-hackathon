...
// JournalV2 — Phase 1 of the Journal redesign. Branches the existing
// 3-column layout into two modes:
//
...
//
//   - Connect: the v1 3-column layout (verse pane + editor + suggestions
//     rail) returns unchanged. Phase 2 will rework Connect's right rail;
//     Phase 3 will rework the verse pane.
//
...
export function JournalV2({ noteId }: Props) {
  ...
  // the persisted per-note mode store. Phase 8 adds "map" to the allowed
  // payload values.
  useEffect(() => {
  ...
  // Phase 8: map mode is full-bleed and shares the user-notes pool with
  // the rest of the app so the JournalMap component can find related
  // entries from both the static SAMPLE_NOTES corpus and any notes the
  // user has created locally. We compute the merged list once per render
  // — `userNotes` is referentially stable between mutations so the
  // identity of the array passed into JournalMap is stable too.
    ...
    <>
      ...
      <div key={note.id} className="journal-v2-pane journal-v2-enter-pane">
        ...
        <div className="journal-v2-doc-wrap">
          <NoteBody
            // Same remount-on-id pattern as the v1 Journal — keeps the
            // editor's local state from leaking between notes.
            key={note.id}
            note={note}
            ...
            // tag row inside NoteBody's V2 metadata stack.
        ...
        {/* Phase 4: edit timestamp pinned to the pane's bottom-right.
            Visibility is driven by the pane's :hover/:focus-within state
            via .journal-v2-edited CSS — no JS needed. */}
...
function ConnectView({ note, linkedAyah, showRail }: ConnectViewProps) {
  return (
    // Phase 1 explicitly preserves the v1 connect layout. We reuse the same
    // `.journal` / `.journal-pane` / `.note-doc-wrap` class hierarchy so the
    // existing CSS handles every measurement here without modification.
